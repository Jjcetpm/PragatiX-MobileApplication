...
class AdminActivityDetailsPage extends StatefulWidget {
  ...
  final bool isCc;
  final int? initialAssignmentTab;
...
class _AdminActivityDetailsPageState extends State<AdminActivityDetailsPage> {
  ...
  int _assignmentTab = 0; // 0: Assigned, 1: Unassigned

  @override
  void initState() {
    super.initState();
    _currentActivity = widget.activity;
    _assignmentTab = widget.initialAssignmentTab ?? (_currentActivity.isAssigned ? 0 : 1);
    _loadDependencies();
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Builder(
              builder: (context) {
                // Calculate total assigned and unassigned across all depts & sections
                int totalAssigned = 0;
                ...
                for (final dept in allDepts) {
                  ...
                  if (rawSections.isNotEmpty) {
                    ...
                    if (_findAssignment(deptId, null, null) != null) {
                      totalAssigned++;
                    } else {
                      totalUnassigned++;
                    }
                ...
                return Column(
                  ...
                  children: [
                    ...
                    // Segmented Toggle for Assigned vs Unassigned
                    Container(
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: InkWell(
                              onTap: () => setState(() => _assignmentTab = 0),
                              borderRadius: BorderRadius.circular(10),
                              child: Container(
                                padding: const EdgeInsets.symmetric(vertical: 8),
                                decoration: BoxDecoration(
                                  color: _assignmentTab == 0 ? Colors.white : Colors.transparent,
                                  borderRadius: BorderRadius.circular(10),
                                  boxShadow: _assignmentTab == 0
                                      ? [
                                          BoxShadow(
                                ...
                                child: Center(
                                  child: Text(
                                    'Assigned Classes ($totalAssigned)',
                                    style: TextStyle(
                                      fontSize: 12.5,
                                      fontWeight: FontWeight.bold,
                                      color: _assignmentTab == 0 ? _primary : const Color(0xFF64748B),
                                    ),
                                  ),
                                ),
                              ),
                          ...
                          Expanded(
                            child: InkWell(
                              onTap: () => setState(() => _assignmentTab = 1),
                              borderRadius: BorderRadius.circular(10),
                              child: Container(
                                padding: const EdgeInsets.symmetric(vertical: 8),
                                decoration: BoxDecoration(
                                  color: _assignmentTab == 1 ? Colors.white : Colors.transparent,
                                  borderRadius: BorderRadius.circular(10),
                                  boxShadow: _assignmentTab == 1
                                      ? [
                                          BoxShadow(
                                            color: Colors.black.withValues(alpha: 0.06),
                                            blurRadius: 4,
                                ...
                                child: Center(
                                  child: Text(
                                    'Unassigned Classes ($totalUnassigned)',
                                    style: TextStyle(
                                      fontSize: 12.5,
                                      fontWeight: FontWeight.bold,
                                      color: _assignmentTab == 1 ? const Color(0xFFDC2626) : const Color(0xFF64748B),
                                    ),
                                  ),
                                ),
                              ),
                    ...
                    Builder(
                      builder: (context) {
                        ...
                        final filteredDepts = allDepts.where((dept) {
                          final deptId = dept['id']?.toString();
                          final rawSections = dept['sections'] as List<dynamic>? ?? [];

                          if (rawSections.isNotEmpty) {
                            if (_assignmentTab == 0) {
                              return rawSections.any((sec) {
                                final secId = sec['id']?.toString();
                                final secName = sec['sectionName'] as String?;
                                return _findAssignment(deptId, secId, secName) != null;
                              });
                            } else {
                              return rawSections.any((sec) {
                                final secId = sec['id']?.toString();
                            ...
                            if (_assignmentTab == 0) {
                              return _findAssignment(deptId, null, null) != null;
                            } else {
                              return _findAssignment(deptId, null, null) == null;
                            }
                          }
                        ...
                        if (filteredDepts.isEmpty) {
                          if (_assignmentTab == 0) {
                            return Container(
                              ...
                              child: Column(
                                children: [
                                  ...
                                  const Text(
                                    'This activity has no faculty assignments configured.',
                                  ...
                                  ElevatedButton.icon(
                                    ...
                                    label: const Text('Assign Staff Now'),
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor: _primary,
                                      foregroundColor: Colors.white,
                                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                            ...
                            return Container(
                              ...
                              child: const Column(
                                children: [
                                  Icon(Icons.check_circle_rounded, size: 40, color: Color(0xFF16A34A)),
                                  SizedBox(height: 8),
                                  Text(
                                    'All Classes Fully Assigned',
                                    style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF166534)),
                                  ),
                                  SizedBox(height: 4),
                                  Text(
                                    'Every department and class section has assigned faculty for this activity.',
                                    style: TextStyle(fontSize: 12.5, color: Color(0xFF15803D)),
                                    textAlign: TextAlign.center,
                                  ),
                                ],
                              ),
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, idx) {
                            ...
                            // Filter sections inside this department based on tab
                            final displayedSections = rawSections.where((sec) {
                              final secId = sec['id']?.toString();
                              final secName = sec['sectionName'] as String?;
                              final hasAssign = _findAssignment(deptId, secId, secName) != null;
                              return _assignmentTab == 0 ? hasAssign : !hasAssign;
                            }).toList();
                                  ...
                                  if (displayedSections.isNotEmpty)
                                    ListView.separated(
                                      ...
                                      itemBuilder: (context, secIdx) {
                                        final sec = displayedSections[secIdx];
                                        final secId = sec['id']?.toString();
